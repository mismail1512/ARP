...
Dynamics::Dynamics()
{
    params = params_manager1.getConfigAsMap();
}
...
void Dynamics::refresh()
{
    ...
    std::map<std::string, float> new_params = params_manager1.getConfigAsMap();
    ...
    try
    {
        ...
        std::cout << "\nparameters change detected: " << std::endl;
        params_manager1.printConfigMap(params);
    }
    catch (const std::out_of_range &e)
    {
        std::cerr << "Error: Missing parameter in config map: " << e.what() << std::endl;
